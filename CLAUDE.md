...
- `analyze.js` — verifies the Firebase ID token manually via WebCrypto (no Admin SDK), checks plan/usage in Firestore via REST, fetches Google News RSS for selected companies (starter/admin only), calls Claude (`claude-sonnet-4-6`, max_tokens 4000 with a `stop_reason === 'max_tokens'` truncation guard), parses the JSON report, increments usage. Accepts optional `jobDescription` (capped 5,000 chars) — when present the prompt adds a `jdMatch` requirement-by-requirement section; an `ats` 6-check section is always produced.
